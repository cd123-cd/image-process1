...
def label_show(label, image):
    ...
    qt_img = QtGui.QImage(qt_img_buf.data, qt_img_buf.shape[1], qt_img_buf.shape[0], QtGui.QImage.Format_RGB32)#存入格式为B,G,R,A 对应 0,1,2,3
    image = QPixmap.fromImage(qt_img).scaled(label.width(), label.height())#是在QImage 上完成图像的所有操作，最后利用 QPixmap 将图像显示在屏幕上
    label.setPixmap(image)#进行图片在控件上的显示
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def print_log(self, log_words):
        current_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())#用于格式化时间，返回以可读字符串表示的当地时间
        self.textLog.append(current_time + ':' + log_words)  # 在指定的区域显示提示信息
        cursor = self.textLog.textCursor()#获得指针对象
    ...
    def initUI(self):
        self.setWindowTitle('基于Python的人脸追踪打码')#构造工具栏, 然后根据工具栏绑定事件
    ...
    def start_detect(self):
        ...
        if self.camera:
            ...
            while True:
                ret, image = self.cap.read()##读取一帧图像 ret 读取了数据就返回True,没有读取数据(已到尾部)就返回False  image 返回读取的视频数据 ——  一帧数据是一个三维数组
                if not ret:
                    break
        ...
        if self.video:
            ...
            while True:
                ...
                label_show(label=self.labelImageOrVideo, image=rectangle)
                cv2.waitKey(int(1000 / self.videoFPS))
            self.cap.release()#来停止捕获视频和关闭相应的显示窗口的
            out.release()
    ...
    def change_video_fps(self):
        number, ok = QInputDialog.getInt(self, "设置fps", "[10-60]")#对话框来获得用户的单个输入信息
